...
def find_closest_boxes(sorted_distances):
...
while True:
    ...
    box_a, box_b = find_closest_boxes(distances[i:])
    i += 1
    ...
    if len(sorted(circuits.items(), key=lambda item: len(item[1]))[-1][1]) == len(coords):
